...
// Multi-timeframe RSI confirmation (2026-07-23, opt-in via --mtf-rsi-
// modifier) - see confidenceModifiers.ts's header comment for why. Resamples
// the SAME 5-min bars already being fetched (no extra data cost, live or
// backtest) into synthetic ~1hr buckets, one close per 12 bars, rather than
// fetching a real higher-timeframe series.
const HTF_BAR_SIZE_BARS = 12
const HTF_RSI_PERIOD = 14

// RSI as of `uptoIndex` (inclusive) on a resampled higher timeframe - no
// lookahead, only ever reads candles[0..uptoIndex]. The still-forming
// current bucket's latest 5-min close is appended as its live reading
// (mirrors TradingView's request.security repainting on the current bar
// rather than waiting for it to close - the email's indicator does the
// same). Returns null until there's enough resampled history for a real
// RSI reading.
const higherTimeframeRsiAt = (candles: Candle[], uptoIndex: number): number | null => {
  const completedBuckets = Math.floor((uptoIndex + 1) / HTF_BAR_SIZE_BARS)
  const htfCloses: number[] = []
  for (let b = 0; b < completedBuckets; b++) {
    htfCloses.push(candles[b * HTF_BAR_SIZE_BARS + HTF_BAR_SIZE_BARS - 1].close)
  }
  const remainder = (uptoIndex + 1) % HTF_BAR_SIZE_BARS
  if (remainder > 0) htfCloses.push(candles[uptoIndex].close)
  if (htfCloses.length <= HTF_RSI_PERIOD) return null
  const rsiSeries = calculateRSI(htfCloses, HTF_RSI_PERIOD)
...
interface PerSymbolBar {
  ...
  // Full rolling window (crosses session boundaries), NOT sessionCandlesSoFar -
  // PBC's swing-extreme lookback needs the same continuously-rolling window
  // MACD_CURL_LOOKBACK_BARS already established the reasoning for (a session-
  // reset lookback misses structure from late in the prior session, found
  // live 2026-07-15). Prototype-only (2026-08-11, --pbc-min-score).
  window: Candle[]
...
const main = async () => {
  ...
  // Joint version, mirrors execute-alerts.ts's hasMomentumResetJoint - checks
  // histogram history across multiple symbols instead of one. Backtest-only
  // representation of the DIV/SPY-QQQ joint gate (2026-08-11, opt-in via
  // --div-joint-reset-gate) - the live DTTF/SPY-IWM version this mirrors
  // (2026-08-07) was never itself run through the backtest before shipping,
  // unlike most other DIV-touching changes here (ADX gate, MTF-RSI, the
  // TTTF momentum-reset test) - this is the first time either joint gate
  // gets a real 90-day read before being trusted.
  const hasMomentumResetJoint = (symbols: Symbol[], direction: 'bullish' | 'bearish', sinceIso: string): boolean => {
    const history = symbols.flatMap(s => histogramHistoryBySymbol[s].filter(h => h.timeIso > sinceIso))
    if (history.length === 0) return false
    return direction === 'bullish' ? history.some(h => h.histogram <= 0) : history.some(h => h.histogram >= 0)
  }
  const DIV_JOINT_RESET_SYMBOLS: Symbol[] = ['SPY', 'QQQ']
  ...
  const attemptGatedEntry = (
    ...
    const tttfGated = ttfStatus === 'TTTF' && tttfMomentumResetGate
    if ((ttfStatus === 'IV' || ttfStatus === 'ORB' || ttfStatus === 'DIV' || tttfGated) && !orbHighConfidenceContinuation) {
    ...
    // DIV joint SPY+QQQ gate, opt-in via --div-joint-reset-gate - layered on
    // top of the per-symbol gate above, not a replacement (mirrors the live
    // 2026-08-11 change exactly).
    if (divJointResetGate && ttfStatus === 'DIV' && (DIV_JOINT_RESET_SYMBOLS as string[]).includes(symbol)) {
      const lastJointClose = DIV_JOINT_RESET_SYMBOLS
        .map(s => lastCloseTimeBySymbolDirection.get(`${s}:${direction}`))
        .filter((d): d is string => !!d)
        .sort()
        .pop()
      if (lastJointClose && nyDateKey(lastJointClose) === nyDateKey(t) && !hasMomentumResetJoint(DIV_JOINT_RESET_SYMBOLS, direction, lastJointClose)) {
        return skip('div_joint_momentum_not_reset')
      }
    }
  ...
  for (const clockBar of clockBars) {
    ...
    for (const symbol of SYMBOLS) {
      ...
      // Trend-day regime filter (2026-07-23, opt-in via --div-adx-trend-gate)
      // - see confidenceModifiers.ts-adjacent reasoning: DIV's thesis is an
      // anticipatory reversal off a decelerating MACD histogram, which reads
      // identically whether it's a real turn or just a shallow pause inside
      // a strong sustained trend (found live 2026-07-23: 10 straight bullish
      // DIV entries into a real gap-down-and-continue morning, all stopped
      // out). ADX measures trend STRENGTH regardless of direction - high
    ...
    if (signalResults.length > 0) {
      ...
      const confidence = applyConfidenceModifiers(baseConfidence, {
        ...
        now,
        chopZoneStartMinutes, chopZoneEndMinutes
        // Found testing 2026-07-23: --mtf-rsi-modifier applied here too
        // (TTTF/DTTF/STTF) helped DIV but meaningfully HURT DTTF (-$1,493
        // over the same window) - scoped to DIV only below instead. See
        // that call site's comment for the isolated DIV-only result.
      })
    ...
    if (!fullConfluenceFired) {
      for (const direction of ['bullish', 'bearish'] as const) {
        ...
        // confidence) fired straight into a gap-down-and-continue morning -
        // all 10 real entries taken that day stopped out. The daily
        // trend (SPY/QQQ/IWM all in a standing multi-month bullish EMA50>
        // EMA200 trend) agreed with DIV's bullish reversal call and got a
        // 1.15x bonus, even though DIV's whole thesis is an ANTICIPATED
        // short-horizon reversal - a signal type that's often, by
        // construction, fighting the immediate move right up until it
        // isn't. The daily trend says nothing about whether THIS bar's dip
        // actually reverses. Opt-in via --div-no-trend-modifier, DIV only -
        // TTTF/DTTF/STTF/IV/ORB unaffected.
        const confidence = applyConfidenceModifiers(baseConfidence, {
          direction,
          dailyEma50: divNoTrendModifier ? null : (dailyLevels?.dailyEma50 ?? null),
          dailyEma200: divNoTrendModifier ? null : (dailyLevels?.dailyEma200 ?? null),
          candlestickDirection: patternMatch?.direction ?? null,
          orbBreakoutDirection: orbDirection,
          vixChangePct: null,
          now,
          chopZoneStartMinutes, chopZoneEndMinutes,
          higherTimeframeRsi: mtfRsiModifier ? higherTimeframeRsiAt(intradayCandles[representative.symbol], representative.globalIndex) : undefined
        })

        // Phase 1 legs stay ungated regardless (matches every other flag in
        // this file - Phase 1 is meant to show what fired regardless of
        // whether the real bot would have traded it). Only Phase 2's real
        // entry attempt is blocked on a trend day.
        const adxTrendDayBlock = divAdxTrendGate !== undefined && representative.adx !== null && representative.adx >= divAdxTrendGate
        ...
        for (const s of divergent) {
          ...
          if (!adxTrendDayBlock) attemptGatedEntry('DIV', s.symbol, direction, confidence, now, s.entryPrice, s.target50EMA, s.globalIndex)
        }
      }
    }
    ...
    // --- 9. PBC (Pullback Confluence, prototype 2026-08-11) ---
    // Doesn't exist at all unless --pbc-min-score is passed. Alert-only in
    // real life (PBC is in DISABLED_SIGNAL_TYPES, api/cron/execute-alerts.ts)
    // - traded here anyway so the backtest can read a real win-rate/entry-
    // count number before it starts alerting daily, same discipline as every
    // other prototype in this file. Single-symbol structural setup, not a
    // cross-index confluence thesis - each symbol scored independently.
    if (pbcMinScore !== undefined) {
      for (const symbol of SYMBOLS) {
        ...
        for (const direction of ['bullish', 'bearish'] as const) {
          const pbc = detectPullbackConfluence(symbolSignal.window, direction, dailyLevels?.dailyEma50 ?? null, dailyLevels?.dailyEma200 ?? null, vwap)
          if (!pbc || pbc.score < pbcMinScore) continue

          const leg = makeLeg('PBC', symbol, direction, pbc.score / 100, now, pbc.entryPrice, pbc.target, symbolSignal.atr, pbc.stopLoss)
          legs.push(leg)
          openLegsBySymbol[symbol].push(leg)
          attemptGatedEntry('PBC', symbol, direction, pbc.score / 100, now, pbc.entryPrice, pbc.target, symbolSignal.globalIndex)
        }
      }
    }
